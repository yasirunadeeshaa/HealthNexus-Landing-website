...
type Pill = { icon: string; text: string; color: string; bg: string };

type PartnerMetric = { label: string; value: string; bg: string; border: string; color: string };

type PartnerBenefit = { icon: string; title: string; desc: string; pill?: Pill };
...
const MetricRow = ({ metrics }: { metrics: PartnerMetric[] }) => (
...
/* ─── Partner Card Body ─── */
const PartnerCardBody = ({ partner }: { partner: Partner }) => (
...
const PartnerCardDesktop = ({ partner }: { partner: Partner }) => {
  const [hovered, setHovered] = useState(false);
  return (
    <div
      onMouseEnter={() => setHovered(true)}
...
/* ─── Partner card (Mobile accordion) ─── */
const PartnerCardMobile = ({ partner }: { partner: Partner }) => {
...
const PARTNERS = [
  {
    ...
    benefits: [
      { icon: '⚡', title: 'Direct prescription pipeline', desc: 'Instant digital prescriptions from 10,000+ verified doctors.', pill: { icon: '📈', text: '50K+ monthly orders', color: T.purple, bg: T.purpleL } },
      { icon: '👥', title: 'Expanded customer base',       desc: 'Access to 2M+ verified patients on the platform.' },
      { icon: '🧠', title: 'AI inventory management',      desc: 'Demand forecasting reduces overstock and shortages.' },
      { icon: '💳', title: 'Automated payments',           desc: 'Instant settlement and reconciliation built in.' },
    ],
  },
  {
    ...
    benefits: [
      { icon: '🏠', title: 'Home collection network', desc: 'Automated scheduling and routing for home sample pickup.', pill: { icon: '📈', text: '15K+ daily collections', color: T.purple, bg: T.purpleL } },
      { icon: '📄', title: 'Digital report delivery',  desc: 'Instant report sharing directly with referring doctors.',  pill: { icon: '✅', text: 'Live integration',       color: T.teal,   bg: T.tealL   } },
      { icon: '🩺', title: 'Doctor collaboration',     desc: 'Direct consultations on results via in-platform messaging.' },
      { icon: '📊', title: 'Analytics dashboard',      desc: 'Test volume trends and demand insights in one view.' },
    ],
  },
  {
    ...
    benefits: [
      { icon: '🤖', title: 'Automated claim processing', desc: 'AI-driven pipeline delivers 90% faster settlements.',          pill: { icon: '⏱️', text: 'Save 15 days/claim', color: T.teal,   bg: T.tealL   } },
      { icon: '🚨', title: 'Fraud detection',            desc: 'Real-time anomaly detection with 99.9% accuracy.',             pill: { icon: '📈', text: '99.9% accuracy',     color: T.purple, bg: T.purpleL } },
      { icon: '✅', title: 'Real-time verification',     desc: 'Instant eligibility checks at point of care.' },
      { icon: '📋', title: 'Paperless workflow',         desc: '100% digital documentation end-to-end.' },
    ],
  },
  {
    ...
    benefits: [
      { icon: '👥', title: 'Patient flow optimisation', desc: 'Smart queue management cuts wait times significantly.',              pill: { icon: '📉', text: '60% less wait time', color: T.amber, bg: T.amberL } },
      { icon: '🗄️', title: 'Unified health records',   desc: 'Centralised patient data with cloud sync across departments.',       pill: { icon: '☁️', text: 'Cloud sync',          color: T.teal,  bg: T.tealL  } },
      { icon: '🔗', title: 'Multi-doctor coordination', desc: 'Seamless referral and handoff management between teams.' },
      { icon: '💰', title: 'Revenue optimisation',      desc: '25% increase in patient volume through platform discovery.' },
    ],
  },
];
  ...
  return (
    <section id="vendorbenefits">
      ...
      <div style={{
        ...
        {/* ── Section header ── */}
        <div id="vb-header" style={{ textAlign: 'center', padding: '72px 24px 48px' }}>
          <div style={{
            display: 'inline-block',
        ...
        {/* ── Info bar ── */}
        <div id="vb-info-bar" style={{ maxWidth: '1100px', margin: '0 auto 40px', padding: '0 24px' }}>
          <div style={{
        ...
        {isMobile ? (
          ...
          <div id="vb-cards-grid" style={{ padding: '0 16px' }}>
            {PARTNERS.map(p => (
              <PartnerCardMobile key={p.title} partner={p} />
            ))}
          </div>
        ) : (
          /* Desktop: 4-column grid */
          <div id="vb-cards-grid" style={{
            ...
            {PARTNERS.map(p => <PartnerCardDesktop key={p.title} partner={p} />)}
          </div>
        )}

        {/* ── Integration timeline ── */}
        <div id="vb-timeline" style={{ maxWidth: '1100px', margin: '40px auto 0', padding: '0 24px 80px' }}>
          <div style={{
            background: '#fff',
            borderRadius: '24px',
            overflow: 'hidden',
            ...
            <div style={{ padding: '28px', position: 'relative' }}>
              <div style={{ position: 'relative', maxWidth: '700px', margin: '0 auto' }}>
                ...
                <div id="vb-connector-line" style={{
